...
module('Integration | Component | project-ui/share-layer', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    assert.dom(`div#embed-modal-${this.rasterLayer.name} input`).hasValue(`<iframe height=600 width=800 src="${window.location.origin}/embed/${this.rasterLayer.name}?base=street&color=%231e88e5" />`);
  });

  test('it updates parameters', async function(assert) {
    await render(hbs`<ProjectUi::ShareLayer @layer={{this.vectorLayer}} />`);
    assert.dom(`div#embed-modal-${this.vectorLayer.name} input`).hasValue(`<iframe height=600 width=800 src="${window.location.origin}/embed/${this.vectorLayer.name}?base=street&color=%231e88e5" />`);
    ...
    assert.dom(`div#embed-modal-${this.vectorLayer.name} input`).hasValue(`<iframe height=600 width=800 src="${window.location.origin}/embed/${this.vectorLayer.name}?base=grayscale&color=%231e88e5" />`);
    await fillIn(`input#layer-${this.vectorLayer.name}`, '#000000');
    await click('button.uk-button-primary');
    assert.dom(`div#embed-modal-${this.vectorLayer.name} input`).hasValue(`<iframe height=600 width=800 src="${window.location.origin}/embed/${this.vectorLayer.name}?base=grayscale&color=%23000000" />`);
  });
});
